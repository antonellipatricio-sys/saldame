...
export function UploadPDFPage() {
  ...
  const [rawText, setRawText] = useState<string | null>(null); // debug
  const [detectedFormat, setDetectedFormat] = useState<'banco-nacion' | 'mercadopago' | null>(null);
  ...
  const handleParse = async () => {
    ...
    try {
      ...
      console.log('[PDF] Texto extraído:', text.slice(0, 500));
      ...
      if (isMercadoPago(text)) {
        ...
        console.log('[PDF] Formato detectado: Banco Nación / VISA / MC');
        setDetectedFormat('banco-nacion');
        transactions = parseTransactions(text);
      }
      console.log('[PDF] Transacciones encontradas:', transactions.length, transactions);

      if (transactions.length === 0) {
        setRawText(text.slice(0, 800));
        setError('No se encontraron transacciones. Revisá el texto extraído abajo para ver qué leyó el parser.');
        setParsing(false);
        return;
      }
  ...
  if (rows.length > 0) {
    return (
      <div className="max-w-4xl mx-auto space-y-5">
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div>
            <div className="flex items-center gap-3">
              <h1 className="text-3xl font-bold text-slate-800">Revisar transacciones</h1>
              {detectedFormat && (
                <span className={cn(
          ...
          <div className="flex gap-3">
            <button onClick={() => { setRows([]); setFile(null); if (inputRef.current) inputRef.current.value = ''; }}
        ...
        {/* Banner de tarjeta Mercado Pago */}
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-5">
      <div>
        <h1 className="text-3xl font-bold text-slate-800">Subir Resumen de Tarjeta</h1>
        <p className="text-slate-500 mt-1 text-sm">Extraemos automáticamente los gastos de tu PDF</p>
      </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 space-y-5">
        ...
        >
          ...
          {!file ? (
            <>
              ...
              <p className="font-medium text-slate-700">Click o arrastrá tu PDF aquí</p>
              <p className="text-sm text-slate-400 mt-1">Resúmenes de tarjeta Visa, Mastercard, AMEX, etc.</p>
            </>
          ) : (
            <>
        ...
        <button onClick={handleParse} disabled={!file || parsing}
          ...
          {parsing
            ? <><Loader2 className="w-5 h-5 animate-spin" /> Leyendo PDF...</>
        ...
        <div className="bg-blue-50 border border-blue-100 rounded-xl p-4 text-sm text-blue-800 space-y-1">
          ...
          <p>1. Subís el PDF de tu resumen de tarjeta</p>
          <p>2. La app detecta automáticamente el formato (Banco Nación, VISA, Mercado Pago)</p>
          <p>3. Se extraen fecha, descripción y monto de cada compra</p>
          <p>4. El clasificador sugiere categoría y etiquetas para cada gasto</p>
          <p>5. Revisás, editás si hace falta, y guardás todo de una vez</p>
          <p className="mt-2 text-xs text-blue-600">
            <strong>Formatos soportados:</strong> Banco Nación / VISA / Mastercard · Mercado Pago
          </p>
        </div>
      </div>
    </div>
